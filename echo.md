...
<head>
  ...
  <title>TEE View & Anatomy Guessing Game</title>
  <style>
    :root{
      --bg: #0b1220;
      --card: #121a2b;
    ...
    *{box-sizing:border-box}
    ...
    body{
      margin:0; background: radial-gradient(1200px 800px at 75% -10%, #1a2440, #0b1220 60%);
    ...
    h1{font-size: clamp(1.3rem, 1.1rem + 1.8vw, 2rem); margin:0}
    .subtitle{color:var(--muted); font-size:.95rem}

    .card{background:linear-gradient(180deg, rgba(255,255,255,.04), rgba(255,255,255,.02)); border:1px solid rgba(255,255,255,.08); border-radius:18px; box-shadow: 0 20px 40px rgba(0,0,0,.35); overflow:hidden}
    ...
    .meta{display:flex; gap:14px; align-items:center; color:var(--text-dim); font-weight:600}
    ...
    .link{background:transparent; border:1px solid rgba(255,255,255,.14); color:var(--text); border-radius:999px; padding:8px 12px; cursor:pointer}
    .link:hover{background:rgba(255,255,255,.06)}

    .toast{position:fixed; left:50%; top:16px; transform:translateX(-50%) translateY(-12px); opacity:0; background:#0f1a2c; color:var(--text); border:1px solid rgba(255,255,255,.1); border-radius:10px; padding:10px 12px; font-weight:600; transition: all .25s ease}
    .toast.show{transform:translateX(-50%) translateY(0); opacity:1}

    /* SVG Arrow styling via CSS variables */
...
<body>
  <div class="app">
    <header>
      <div class="title">
        ...
        <span class="subtitle">Identify the labelled structure to advance</span>
      </div>
      <div class="meta">
    ...
    <section class="card">
      <div class="stage">
        <div id="imageWrap" class="image-wrap">
          ...
          <!-- Overlay elements inserted dynamically -->
  ...
  <script>
     ...
     * ==============================
     * For best results, supply real TEE images and precise target positions.
     * - image: URL or relative path to your anonymized echo image/loop poster
     * - target: where the arrow should point (percent of image width/height)
     * - choices: answer options shown as buttons
     * - answer: the correct text (must match one item in choices)
     */
    const QUESTIONS = [
      {
    ...
    function loadQuestion(){
      ...
      // Once image loads, draw arrow overlay
    ...
    function redrawOverlay(){
      ...
      // Choose a start point offset toward top-left but clamped to bounds
      ...
      // Optional hint dot at target (small pulsing point)
    ...
    function placeLabel(text){
      // Put label near the start of the arrow for readability
      const wrap = $("#imageWrap");
      const rect = wrap.getBoundingClientRect();
      const q = QUESTIONS[order[index]];
      const x2 = rect.width * (q.target.x/100);
